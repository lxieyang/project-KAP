export default function HighlightSelector({ onTrigger, timer }) {
  return {
    conditions: function({ mouseDown }) {
      return (
        mouseDown &&
        !(
          mouseDown.getModifierState("Alt") ||
          mouseDown.getModifierState("Meta")
        ) &&
        ["INPUT", "TEXTAREA"].indexOf(mouseDown.target.nodeName) < 0 &&
        !mouseDown.target.isContentEditable
      );
    },
    ...
    onSelectionEnd: function(e) {
      ...
      if (e.mouseUp) {
        //We ended the selection because the mouse is now up
